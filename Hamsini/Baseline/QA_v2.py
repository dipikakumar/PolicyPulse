...
def model_fn(model_dir):
    ...
    # Use the provided model directory
    s3_model_path = os.path.join(model_dir, "models/mistral")
...
def fn_local_with_paths(input_data: Dict, bucket_name) -> Tuple[Optional[str], Optional[str], str, str, str]:
    ...
    company_name = input_data.get("company_name", "").strip()  # Keep this for logs and context
...
def predict_fn(input_data, bucket_name,  pipeline_components, sample_question, model=None):
    """
    ...
    base_embeddings = pipeline_components["embeddings"]
    qdrant_url = pipeline_components["qdrant_url"]
    qdrant_key = pipeline_components["qdrant_key"]
    openai_api_key = pipeline_components["open_api_key"]
    mistral_llm_lc = pipeline_components["llm"]
    ...
    try:
        ...
        # Log the retrieved paths and metadata
        ...
        if question_type == "Policy":
            ...
            #print(f"Total pages in Policy Document ({Path(policy_paths).name}): {len(policy_contents)}")
        ...
        # print("\nReturning the following to the UI:")
        # print()
        # print(result['answer'])
        # print()
...
def output_fn(prediction, content_type):
    ...
    if content_type == "application/json":
        try:
            # Serialize the plain string directly
...
def main():
    ...
    prediction = predict_fn(input_data, bucket_name, pipeline_components, sample_question)
